...
(function () {
  ...
  // ---- Estudo coletivo -------------------------------------------------
  // Grupo próprio desta página (coleção "groups-leis"), independente do
  // grupo do Diário dos Informativos — mesmo padrão, código separado.
  var GROUP_KEY = "leis-grupo";
  ...
  function memberDocRef(code) {
    return dbCap ? dbCap.doc("groups-leis/" + code + "/members/" + viewerId) : null;
  }
  ...
  function fetchGroupCreator(code) {
    if (!dbCap) return;
    dbCap.doc("groups-leis/" + code).get().then(function (snap) {
      if (snap.exists) {
        var data = snap.data();
  ...
  function subscribeGroup(code) {
    ...
    if (!dbCap) return;
    groupMembersUnsub = dbCap.collection("groups-leis/" + code + "/members").onSnapshot(
      renderLeaderboard,
      function () {}
    );
  ...
  function createGroup() {
    ...
    dbCap.doc("groups-leis/" + code).set({ createdAt: new Date().toISOString(), createdBy: viewerId })
      .then(function () {
