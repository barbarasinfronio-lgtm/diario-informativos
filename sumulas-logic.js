...
(function () {
  ...
  // ---- Estudo coletivo --------------------------------------------------
  // Mesmo grupo compartilhado dos outros dois Diários — quem entra em
  // qualquer um dos três já faz parte dos três.
  var GROUP_KEY = "informativos-grupo";
  function readGroupPref() {
    try { var raw = localStorage.getItem(GROUP_KEY); return raw ? JSON.parse(raw) : null; }
    catch (e) { return null; }
  }
  function writeGroupPref(pref) {
    try { if (pref) localStorage.setItem(GROUP_KEY, JSON.stringify(pref)); else localStorage.removeItem(GROUP_KEY); }
    catch (e) {}
  }
  ...
  function updateMyMemberDoc() {
    ...
    if (!ref) return;
    // merge: true — este membro pode já ter campos dos outros dois Diários
    // (lidas/estrelasOuro do de Informativos, lidasLeis do de Leis); só
    // atualizamos o campo próprio deste Diário, sem apagar os outros.
    ref.set({
      name: groupPref.name,
  ...
  function renderCachedLeaderboard() { if (lastGroupSnap) renderLeaderboard(lastGroupSnap); }
  ...
  function renderLeaderboard(snap) {
    ...
    var members = snap.docs.map(function (d) {
      var data = d.data() || {};
      return { id: d.id, name: data.name, lidas: data.lidasSumulas, avatar: data.avatar, joinedAt: data.joinedAt || "" };
    ...
    members.forEach(function (m) {
      ...
      li.appendChild(nameEl); li.appendChild(emojisEl); li.appendChild(countEl);
  ...
  function subscribeGroup(code) {
    ...
    if (!dbCap) return;
    groupMembersUnsub = dbCap.collection("groups/" + code + "/members").onSnapshot(renderLeaderboard, function () {});
  ...
  function createGroup() {
    ...
    dbCap.doc("groups/" + code).set({ createdAt: new Date().toISOString(), createdBy: viewerId })
      .then(function () {
        ...
        writeGroupPref(groupPref);
        renderGroupUI(); subscribeGroup(code); updateMyMemberDoc();
      })
  ...
  function leaveGroup() {
    if (groupMembersUnsub) { groupMembersUnsub(); groupMembersUnsub = null; }
    if (dbCap && groupPref) { var ref = memberDocRef(groupPref.code); if (ref) ref.delete().catch(function () {}); }
    groupPref = null; writeGroupPref(null); groupCreatorId = null; lastGroupSnap = null;
  ...
  function bindUser(uid) {
    ...
    if (groupPref) { subscribeGroup(groupPref.code); updateMyMemberDoc(); }
  }
